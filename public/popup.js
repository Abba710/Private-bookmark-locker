...
function showSupportDialog() {
  ...
  const donateDiv = donateDialog.querySelector("div");

  donateDialog.classList.remove("hidden");
  donateDialog.classList.add("flex");

  // Trigger animation
  requestAnimationFrame(() => {
    donateDiv.classList.remove("scale-95", "opacity-0");
    donateDiv.classList.add("scale-100", "opacity-100");
  });
}
...
function hideSupportDialog() {
  const donateDialog = document.getElementById("donateDialog");
  const donateDiv = donateDialog.querySelector("div");
  donateDiv.classList.remove("scale-100", "opacity-100");
  donateDiv.classList.add("scale-95", "opacity-0");
  setTimeout(() => {
    donateDialog.classList.remove("flex");
    donateDialog.classList.add("hidden");
        ...
        if (!bookmarks.some((b) => b.url === url)) {
          ...
          chrome.storage.local.set(
            ...
            () => {
              ...
              if (bookmarkCount > 0 && bookmarkCount % 21 === 0) {
                showSupportDialog();
              }
            }
          );
...
function displayBookmarks(shouldScroll = false) {
  trunOffInstructions();
  chrome.storage.local.get(["bookmarks", BOOKMARK_SWITCH_KEY], (result) => {
    ...
    bookmarks.forEach((bookmark, index) => {
      if (bookmark.incognito === switchState) {
        ...
        li.className =
          "flex justify-start items-center bg-[#25262b] w-[295px] max-h-[40px] p-2 overflow-hidden rounded-xl hover:bg-[#2c2d33] text-gray-300 hover:text-[#45E3B8] transition-all duration-300 shadow-md shadow-black/10 hover:shadow-lg hover:shadow-black/20 border border-gray-700/20 hover:border-gray-700/30 group cursor-pointer mb-3";
        li.draggable = true;
        const link = document.createElement("link");
        const container = document.createElement("div");
        container.className =
          "flex items-start justify-start min-w-0 p-[2px] min-w-0";

        const text = document.createElement("div");
        text.textContent = bookmark.title || bookmark.url;
        text.title = bookmark.url;
        text.className =
          "truncate text-base text-left transition-all duration-300 hover:translate-x-1 font-medium";

        container.appendChild(text);

        const dragIcon = document.createElement("i");
        dragIcon.className = "uil uil-draggabledots text-gray-400/70";

        const delBtn = document.createElement("button");
        delBtn.innerHTML = "🗑️";
        delBtn.className =
          "text-red-400/70 ml-auto hover:text-red-500 text-xl transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 active:scale-90";
    ...
    if (shouldScroll && list.lastElementChild) {
