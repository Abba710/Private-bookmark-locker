      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-[400px] bg-[#0f0f11] border border-white/10 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <button
              onClick={() => setModalOpen(false)}
              className="absolute top-5 right-5 cursor-pointer p-2 text-zinc-500 hover:text-white hover:bg-white/5 rounded-full transition-colors z-10"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="relative p-8 flex flex-col items-center">
              ...
              <button
                onClick={() => setModalOpen(false)}
                className="w-full py-4 rounded-xl cursor-pointer bg-white/5 border border-white/5 text-white text-sm font-semibold hover:bg-white/10 hover:border-white/10 transition-all active:scale-[0.98]"
              >
